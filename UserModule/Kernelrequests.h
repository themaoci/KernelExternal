...
#pragma warning( pop )
	...
	void createSecuritydesc() {
		...
		// https://docs.microsoft.com/en-us/windows/desktop/api/accctrl/ns-accctrl-_explicit_access_a - 404
	...
	void CreateSharedEvents() {
		...
		SharedEvent_trigger = CreateEventA(&sa, TRUE, FALSE, globTrigger);
		...
		SharedEvent_ready2read = CreateEventA(&sa, TRUE, FALSE, globReadyRead);
	...
	bool OpenSharedMemory() {
		hMapFileW = OpenFileMappingA(FILE_MAP_WRITE, FALSE, globSharedMem);
		if (!hMapFileW || hMapFileW == INVALID_HANDLE_VALUE)
		{
#ifdef DBG_PRINT
		...
		hMapFileR = OpenFileMappingA(FILE_MAP_READ, FALSE, globSharedMem);
